...
data.folder <- "C:/Data/suoajot36_47/data"
...
ID.map.data$Suojelukoodi <- factor(ID.map.data$SKoodi, 
...
variant.48 <- get_variant(project.suoz, 7)
...
p1 + geom_vline(xintercept=0.513)
...
p5 <- plot(variant.42, monochrome=FALSE, group=TRUE, labels=labels,
           statistic="mean", invert.x=TRUE, main="42 - koko maa")
p5  <- p5 + theme(legend.position=c(.25, .2))
